...
export const courses = pgTable(
  ...
  (table) => ({
    ownerTitle: uniqueIndex("course_owner_title_idx").on(table.ownerId, table.title),
...
export const lessons = pgTable(
  ...
  (table) => ({
    courseOrder: uniqueIndex("lesson_course_order_idx").on(table.courseId, table.orderIndex),
...
export const reviewStates = pgTable("review_state", {
  cardId: uuid("card_id").primaryKey().references(() => cards.id, { onDelete: "cascade" }),
...
export const reviewLogs = pgTable(
  ...
  (table) => ({
    cardReviewedAt: index("review_log_card_reviewed_at_idx").on(table.cardId, table.reviewedAt)
  })
);
